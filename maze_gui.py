...
class main():
    def __init__(self, root):
        ...
        self.entry2.pack(pady=5)
         # Tạo Label và Entry widget cho số nguyên thứ hai
        Label(self.root, text="Input position x of goal:").pack(pady=5)
        self.entry3 = Entry(self.root)
        self.entry3.pack(pady=5)
         # Tạo Label và Entry widget cho số nguyên thứ hai
        Label(self.root, text="Input position y of goal:").pack(pady=5)
...
class MazeGUI:
    ...
    def __init__(self, rows: int = get_r(), columns: int = get_c(), sparseness: float = 0.2,
                 start: MazeLocation = MazeLocation(0, 0), goal: MazeLocation = MazeLocation(get_p_r(), get_p_r())) -> None:
    ...
    def step(self, frontier, explored, last_node):
        if not frontier.empty:
            ...
            self._display_grid()
            num_delay = int(self._interval_box.get()) * 500
